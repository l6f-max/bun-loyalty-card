...
let S = {
  ...
  scanning: false,
  scanStatus: "",
  showRewardsHistory: false,
  showStampsList: false,
  loadingRewardsHistory: false,
  rewardsHistory: [],
  justStampedId: null,
...
async function redeemReward(id){
  ...
  if(!updated) return;
  Object.assign(c, updated);
  // نسجّل الاستبدال بسجل منفصل عشان يظهر بتاريخ المكافآت (لو الجدول موجود، وإلا نتجاهل الخطأ بصمت)
  try{
    await window.supabase.rpc("log_redemption", { p_customer_id: id, p_stamps_used: required });
    if(S.showRewardsHistory) loadRewardsHistory();
  }catch(e){ /* الجدول لسا ما انضاف بقاعدة البيانات — تجاهل */ }
  render();
  toast("تم استبدال المكافأة 🎉");
}

async function loadRewardsHistory(){
  S.loadingRewardsHistory = true;
  render();
  const { data, error } = await window.supabase.rpc("get_my_cafe_redemptions");
  S.loadingRewardsHistory = false;
  if(error){
    S.rewardsHistory = [];
    toast("سجل الاستبدال غير مفعّل بعد بقاعدة البيانات — راجع تعليمات الإضافة");
    render();
    return;
  }
  S.rewardsHistory = data || [];
  render();
}

async function undoRedemptionAction(redemptionId){
  const { data, error } = await window.supabase.rpc("undo_redemption", { p_redemption_id: redemptionId });
  if(error){ toast(error.message || "تعذّر التراجع عن الاستبدال"); return; }
  // نحدّث بيانات العميل المحلية لو موجود بالقائمة
  if(data){
    const c = S.customers.find(c=>c.id===data.id);
    if(c) Object.assign(c, data);
  }
  S.rewardsHistory = S.rewardsHistory.filter(r=>r.id!==redemptionId);
  render();
  toast("تم التراجع عن الاستبدال، رجّعنا الطوابع للعميل ✓");
}

async function clearAllStampsFor(id){
  const c = S.customers.find(c=>c.id===id);
  if(!c || c.stamps<=0) return;
  const name = c.name;
  while(c.stamps > 0){
  ...
  render();
  toast(`تم حذف طوابع ${name} ✓`);
}

async function clearRewardsFor(id){
  const c = S.customers.find(c=>c.id===id);
  if(!c || c.rewards<=0) return;
  const name = c.name;
  const { data, error } = await window.supabase.rpc("reset_customer_rewards", { p_customer_id: id });
  if(error){ toast("تعذّر تصفير المكافآت: " + error.message); return; }
  Object.assign(c, data);
  render();
  toast(`تم تصفير مكافآت ${name} ✓`);
}
...
function applyBranding(cafe){
  const name = (cafe && cafe.name) ? cafe.name : "طوابع";
  const icon = (cafe && cafe.logo_url) ? cafe.logo_url : (window.location.origin + "/icon-192.png");
  const icon512 = (cafe && cafe.logo_url) ? cafe.logo_url : (window.location.origin + "/icon-512.png");
  ...
  if(touchIcon) touchIcon.setAttribute("href", icon);

  try{
    const manifest = {
      name, short_name: name,
      start_url: window.location.pathname + window.location.search,
      scope: window.location.pathname + window.location.search,
      display: "standalone",
...
function walletIconGrid(stamps, icon, total){
  icon = icon || "☕";
  total = total || 10;
  let html = '<div class="wstrip">';
  for(let i=0;i<total;i++){
    const cls = i < stamps ? "wchip filled" : "wchip empty";
    html += `<div class="wicon"><div class="${cls}">${icon}</div></div>`;
  }
  html += '</div>';
  return html;
}
...
function render(){
    ...
    if(!S.authed){
      ...
      html += `<div class="section">
        ...
        <div class="stats">
          ...
          <button data-action="toggleStampsList" style="all:unset;cursor:pointer;flex:1;min-width:120px;">
            ${statCard("📈","طوابع موزّعة (اضغط للتفاصيل)",stats.totalStamps)}
          </button>
          <button data-action="toggleRewardsHistory" style="all:unset;cursor:pointer;flex:1;min-width:120px;">
            ${statCard("🎁","مكافآت مستبدلة (اضغط للتفاصيل)",stats.totalRewards)}
          </button>
        </div>
        ...
        <div class="card">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
            <div style="font-weight:700;font-size:14px;">📈 العملاء اللي عندهم طوابع</div>
            <button class="btn-outline" data-action="refreshList">🔄 تحديث</button>
          </div>
          ${(()=>{ const withStamps = S.customers.filter(c=>c.stamps>0); return withStamps.length ? `
            <div style="display:flex;flex-direction:column;gap:8px;">
              ${withStamps.map(c=>`
                <div style="display:flex;justify-content:space-between;align-items:center;border:1px solid var(--kraftDark);border-radius:8px;padding:8px 10px;">
                  <div>
                    <div style="font-weight:700;font-size:13px;">${c.name}</div>
                    <div class="muted">كود: ${c.code} — ${c.stamps} طابع</div>
                  </div>
                  <button class="btn-outline" style="color:var(--ink);border-color:var(--ink);" data-action="clearStamps" data-id="${c.id}">🗑️ حذف الطوابع</button>
                </div>
              `).join("")}
            </div>
          ` : `<p class="muted">لا يوجد عملاء عندهم طوابع حاليًا.</p>`; })()}
        </div>
        ` : ""}

        ${S.showRewardsHistory ? `
        <div class="card">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
            <div style="font-weight:700;font-size:14px;">🎁 سجل المكافآت المستبدلة</div>
            <button class="btn-outline" data-action="refreshRewardsHistory">🔄 تحديث</button>
          </div>
          ${S.loadingRewardsHistory ? `<p class="muted">جاري التحميل...</p>` : S.rewardsHistory.length ? `
            <div style="display:flex;flex-direction:column;gap:8px;">
              ${S.rewardsHistory.map(r=>`
                <div style="display:flex;justify-content:space-between;align-items:center;border:1px solid var(--kraftDark);border-radius:8px;padding:8px 10px;">
                  <div>
                    <div style="font-weight:700;font-size:13px;">${r.customer_name}</div>
                    <div class="muted">${new Date(r.redeemed_at).toLocaleString('ar-SA')}</div>
                  </div>
                  <button class="btn-outline" style="color:var(--ink);border-color:var(--ink);" data-action="undoRedemption" data-id="${r.id}">🗑️ تراجع</button>
                </div>
              `).join("")}
            </div>
          ` : `<p class="muted">لا يوجد سجل استبدال مسجّل بالتفصيل بعد (بيبدأ يتسجّل من أول عملية استبدال جديدة بعد التحديث).</p>`}
        </div>

        <div class="card">
          <div style="font-weight:700;font-size:14px;margin-bottom:8px;">👤 مين عنده مكافآت مستبدلة الآن</div>
          ${(()=>{ const withRewards = S.customers.filter(c=>c.rewards>0); return withRewards.length ? `
            <div style="display:flex;flex-direction:column;gap:8px;">
              ${withRewards.map(c=>`
                <div style="display:flex;justify-content:space-between;align-items:center;border:1px solid var(--kraftDark);border-radius:8px;padding:8px 10px;">
                  <div>
                    <div style="font-weight:700;font-size:13px;">${c.name}</div>
                    <div class="muted">كود: ${c.code} — ${c.rewards} مكافأة مستبدلة</div>
                  </div>
                  <button class="btn-outline" style="color:var(--ink);border-color:var(--ink);" data-action="clearRewards" data-id="${c.id}">🗑️ تصفير المكافآت</button>
                </div>
              `).join("")}
            </div>
          ` : `<p class="muted">لا يوجد عملاء عندهم مكافآت مستبدلة حاليًا.</p>`; })()}
        </div>
        ` : ""}
        ...
        <div class="card">
          ...
          ${S.customers.length ? `
          <button class="btn-outline" style="width:100%;margin-top:10px;" data-action="exportCustomers">📊 تحميل بيانات العملاء (Excel)</button>
          ` : ""}
        </div>
  ...
  if(S.view === "customer"){
    ...
    const smallLogo = (S.customerCafe && S.customerCafe.logo_url)
      ? `<img src="${S.customerCafe.logo_url}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:10px;" />`
      : `${(S.customerCafe&&S.customerCafe.stamp_icon)||"☕"}`;

    if(S.cafeError){
      html += `<div class="section"><div class="card center"><p class="muted">${S.cafeError}</p></div></div>`;
    } else if(!S.session){
      html += `<div class="section" style="padding-top:32px;">
        <div class="card center">
      ...
      const remaining = Math.max(0, required - m.stamps);
      html += `<div class="section" style="padding-top:24px;">
        <div class="wallet-card">
          <div class="wtop">
            <div class="wlogo">${smallLogo}</div>
            <div style="display:flex;align-items:center;gap:8px;">
              <div class="wbrand">${headerLabel}</div>
              <button class="wrefresh" data-action="refreshMe" aria-label="تحديث">🔄</button>
            </div>
          </div>
          ${walletIconGrid(m.stamps, myIcon, required)}
          <div class="wgreet">
            <div class="wname">مرحبًا<b>${m.name}</b></div>
            <div class="wremain"><span>${remaining>0 ? "متبقي على هديتك" : "هديتك جاهزة"}</span><b>${remaining>0 ? remaining : "🎉"}</b></div>
          </div>
          <div class="wqrsec">
            <div class="muted" style="margin-bottom:10px;">اعرض هذا الكود للكاشير عند الطلب</div>
            <div class="qrbox">${qrSvg(m.code)}</div>
            <div class="code-pill" style="margin-top:10px;display:inline-block;">${m.code}</div>
          </div>
        </div>
        ${m.stamps>=required ? `<div class="reward-ready">✨ مكافأتك جاهزة — أرِها للكاشير</div>` : ""}
...
function exportCustomersCSV(){
  if(!S.customers.length){ toast("لا يوجد عملاء لتصديرهم"); return; }
  const headers = ["الاسم","الجوال","الكود","الطوابع الحالية","إجمالي الطوابع","المكافآت المستبدلة"];
  const escCell = (v)=>{
    const s = (v===null || v===undefined) ? "" : String(v);
    return /[",\n]/.test(s) ? `"${s.replace(/"/g,'""')}"` : s;
  };
  const rows = S.customers.map(c => [
    c.name, c.phone||"", c.code, c.stamps, c.total_stamps, c.rewards
  ].map(escCell).join(","));
  ...
  const cafeName = (S.cafe && S.cafe.name) ? S.cafe.name.replace(/[^\u0621-\u064Aa-zA-Z0-9 _-]/g,"") : "عملاء";
  a.href = url;
  a.download = `عملاء-${cafeName}.csv`;
  ...
  root.querySelectorAll("[data-action]").forEach(el=>{
    ...
    el.addEventListener("click", ()=>{
      switch(action){
        ...
        case "exportCustomers": exportCustomersCSV(); break;
        case "toggleRewardsHistory":
          S.showRewardsHistory = !S.showRewardsHistory;
          if(S.showRewardsHistory) loadRewardsHistory(); else render();
          break;
        case "refreshRewardsHistory": loadRewardsHistory(); break;
        case "undoRedemption": undoRedemptionAction(el.dataset.id); break;
        case "toggleStampsList": S.showStampsList = !S.showStampsList; render(); break;
        case "clearStamps": clearAllStampsFor(el.dataset.id); break;
        case "clearRewards": clearRewardsFor(el.dataset.id); break;
